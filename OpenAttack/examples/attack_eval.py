...
class AttackEval:
    ...
    def eval(self, ori_file_path, dataset: Iterable[Dict[str, Any]], total_len : Optional[int] = None, visualize : bool = False, progress_bar : bool = False, num_workers : int = 0, chunk_size : Optional[int] = None):
        """
        ...
        total_inst = 0
        success_inst = 0
        return_file_path = ori_file_path.split('.')[0] + '_adv.' + ori_file_path.split('.')[1]
        return_file = open(return_file_path, "a+", encoding="utf-8")
            ...
            if visualize and (TAG_Classification in self.victim.TAGS):
                ...
                if res["success"]:
                    ...
                    if Tag("get_prob", "victim") in self.victim.TAGS:
                        ...
                        raise RuntimeError("Invalid victim model")
                return_file.write(json.dumps({"task_id": f"HumanEval/{i}", "prompt": pre_data}, ensure_ascii=False)+'\n')
                info = res["metrics"]
        ...
        return_file.write(f"Successful Instances: {success_inst}")
        return_file.close()
